...
ApplicationWindow {
    ...
    // 主体：侧栏 + 页面区。上下两块的切分用 anchors 而不是再套一层 Layout ——
    // 嵌在 Layout 里的 Layout 会忽略自己写的 fill 标记，原因见 Sidebar.qml 顶部。
    RowLayout {
        anchors.left: parent.left
        anchors.right: parent.right
        ...
        // 页面区：Item 只负责在 RowLayout 里占掉剩下的空间，排版交给里面的 ColumnLayout
        Item {
            Layout.fillWidth: true
